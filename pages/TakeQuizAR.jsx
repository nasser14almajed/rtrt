...
import { QuizAR, QuestionAR, SubmissionAR, QuestionBankAR, SectionAR } from "@/api/entities";
...
export default function TakeQuizAR() {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      if (!token) {
        setError("رابط الاختبار غير صالح أو مفقود.");
        setIsLoading(false);
        return;
      }

      try {
        const quizzes = await QuizAR.filter({ share_token: token });
        if (quizzes.length === 0) {
          setError("الاختبار غير موجود. قد يكون الرابط غير صالح أو تم إلغاء نشر الاختبار.");
          setIsLoading(false);
          return;
        }
        ...
        if (!loadedQuiz.owner_id) {
          setError("هذا الاختبار قديم ولا يمكنه قبول الإجابات. يرجى الاتصال بمنشئ الاختبار واطلب منه إعادة نشره لتطبيق آخر تحديثات الأمان.");
          setIsLoading(false);
          return;
        }

        if (loadedQuiz.status !== 'published') {
          setError("هذا الاختبار غير نشط حاليًا.");
          setIsLoading(false);
          return;
        }
        ...
        const savedSessionJSON = localStorage.getItem(`quizSession_AR_${loadedQuiz.id}_${currentIp}`);
        if (savedSessionJSON) {
          try {
            const sessionData = JSON.parse(savedSessionJSON);
            if (sessionData.assignedQuestions && sessionData.assignedQuestions.length > 0) {
              const confirmResume = window.confirm("تم العثور على اختبار قيد التقدم. هل تريد المتابعة من حيث توقفت؟");
              ...
              if (confirmResume) {
                ...
                if (loadedQuiz.settings?.time_limit && sessionData.startTime) {
                  ...
                  if (newTimeRemaining === 0) {
                    alert("انتهى الوقت! سيتم إرسال اختبارك تلقائيًا.");
                    handleTimeUp();
                    return;
                  }
                }

                alert("تم استئناف الجلسة من حيث توقفت!");
                setQuizState("active");
                setIsLoading(false);
                return;
              } else {
                localStorage.removeItem(`quizSession_AR_${loadedQuiz.id}_${currentIp}`);
              }
            }
          } catch (parseError) {
        ...
        if (loadedQuiz.settings?.use_question_bank) {
          const sectionsData = await SectionAR.filter({ owner_id: loadedQuiz.owner_id });
        ...
        setError("حدث خطأ أثناء تحميل الاختبار. يرجى المحاولة مرة أخرى لاحقًا.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const saveSessionData = () => {
    if (quiz && assignedQuestions.length > 0 && respondentInfo.id_number && ipAddress) {
      ...
      try {
        localStorage.setItem(`quizSession_AR_${quiz.id}_${ipAddress}`, JSON.stringify(sessionData));
      } catch (e) {
  ...
  const loadQuizQuestions = async () => {
    if (!quiz || !respondentInfo.id_number || !ipAddress) {
      setError("معلومات غير مكتملة لتحميل الأسئلة.");
      setQuizState("info");
      return;
    }
    ...
    try {
      if (quiz.settings?.use_question_bank) {
        ...
        if (quizSettings.section_distribution && quizSettings.section_distribution.length > 0) {
          const questionPromises = quizSettings.section_distribution.map(async (dist) => {
            ...
            if (dist.section_id === 'uncategorized') {
              const allUserQuestions = await QuestionBankAR.filter({ owner_id: quiz.owner_id });
              sectionQuestions = allUserQuestions.filter(q => !q.section_id);
            } else {
              sectionQuestions = await QuestionBankAR.filter({ owner_id: quiz.owner_id, section_id: dist.section_id });
            }
          ...
          for (const secId in sectionCounts) {
            ...
            if (secId === 'uncategorized') {
              currentQuizSectionsInfo.push({ name: "غير مصنف", count: count, color: "gray" });
            } else {
          ...
          let bankQuestions = [];
          if (sectionIds.length > 0) {
            bankQuestions = await QuestionBankAR.filter(query);
          }
          ...
          if (allSelectedQuestions.length > 0) {
            currentQuizSectionsInfo.push({ name: "الأسئلة المختارة", count: allSelectedQuestions.length, color: "blue" });
          }
        }
        ...
        loadedQuestions = await QuestionAR.filter({
          quiz_id: quiz.id,
          owner_id: quiz.owner_id
        }, 'order', 500);

        if (!Array.isArray(loadedQuestions) || loadedQuestions.length === 0) {
          setError("هذا الاختبار لا يحتوي على أسئلة حاليًا. يرجى الاتصال بمنشئ الاختبار.");
          setQuizState("info");
          return;
        }
        ...
        if (loadedQuestions.length > 0) {
          currentQuizSectionsInfo.push({ name: "أسئلة الاختبار", count: loadedQuestions.length, color: "blue" });
        }
      }
      ...
      setError("حدث خطأ أثناء تحميل أسئلة الاختبار. يرجى المحاولة مرة أخرى لاحقًا.");
      setQuizState("info");
    }
  };
  ...
  const handleStartQuiz = async (e) => {
    ...
    if (!respondentInfo.name.trim()) {
      currentFormErrors.name = "الاسم مطلوب.";
      hasErrors = true;
    }
    if (!respondentInfo.id_number.trim()) {
      currentFormErrors.id_number = "رقم الهوية/التعريف مطلوب.";
      hasErrors = true;
    }
    ...
    try {
      if (quiz.settings?.restrict_by_ip && ipAddress) {
        const ipSubmissions = await SubmissionAR.filter({ quiz_id: quiz.id, ip_address: ipAddress });
        if (ipSubmissions.length > 0) {
          setError("تم تسجيل إجابة بالفعل من هذا الجهاز. لا يسمح بالمحاولات المتعددة.");
          return;
        }
      }

      if (quiz.settings?.restrict_by_id && respondentInfo.id_number.trim()) {
        const idSubmissions = await SubmissionAR.filter({
          quiz_id: quiz.id,
          respondent_id_number: respondentInfo.id_number.trim(),
        });
        if (idSubmissions.length > 0) {
          setError("تم استخدام رقم الهوية هذا بالفعل لإكمال هذا الاختبار. لا يسمح بإعادة الاختبار من نفس الهوية.");
          return;
        }
      }
    } catch (err) {
      console.error("Error checking for duplicate submissions:", err);
      setError("حدث خطأ أثناء التحقق من المحاولات السابقة. يرجى المحاولة مرة أخرى.");
      return;
    }
  ...
  const handleSubmit = async (isAutoSubmit = false) => {
    ...
    if (!isAutoSubmit && unansweredRequiredQuestions.length > 0) {
      alert(`يرجى الإجابة على جميع الأسئلة المطلوبة قبل الإرسال.`);
    ...
    if (!isAutoSubmit && !confirm("هل أنت متأكد من أنك تريد إنهاء الاختبار وإرسال إجاباتك؟")) {
      return;
    }
    ...
    try {
      await SubmissionAR.create(submissionData);

      if (quiz && respondentInfo.id_number && ipAddress) {
        const localStorageKey = `quizSession_AR_${quiz.id}_${ipAddress}`;
      ...
      setError("فشل في إرسال الاختبار. يرجى المحاولة مرة أخرى.");
      setQuizState("active");
  ...
  const handleTimeUp = () => {
    alert("انتهى الوقت! سيتم إرسال اختبارك تلقائيًا مع الإجابات التي قدمتها حتى الآن.");
    handleSubmit(true);
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100" dir="rtl">
        <div className="text-center">
          ...
          <p className="text-slate-600 text-lg">جاري تحميل الاختبار...</p>
          <p className="text-slate-500 text-sm mt-2">يرجى الانتظار</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 overflow-hidden">
      <AnimatePresence mode="wait">
          ...
          >
            <Card className="w-full max-w-md shadow-lg">
              <CardHeader className="text-center">
                ...
                <CardTitle className="text-2xl font-bold text-slate-800">اختبار محمي</CardTitle>
                <p className="text-slate-600 mt-2">يتطلب هذا الاختبار كلمة مرور للوصول</p>
              </CardHeader>
            ...
            <Card className="w-full max-w-2xl shadow-lg">
              ...
              <CardContent className="space-y-6">
                <form onSubmit={handleStartQuiz} className="space-y-4">
                  <h3 className="text-lg font-semibold text-center">الرجاء إدخال معلوماتك</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="name" className="flex items-center gap-1 mb-1">
                        <User className="w-3 h-3"/> الاسم الكامل <span className="text-red-500">*</span>
                      </Label>
                      <Input
                        id="name"
                    ...
                    <div>
                      <Label htmlFor="id_number" className="flex items-center gap-1 mb-1">
                        <IdCard className="w-3 h-3"/> رقم الهوية/التعريف <span className="text-red-500">*</span>
                      </Label>
                      <Input
                        id="id_number"
                  ...
                  <div>
                    <Label htmlFor="phone" className="flex items-center gap-1 mb-1">
                      <Phone className="w-3 h-3"/> رقم الهاتف (اختياري)
                    </Label>
                  ...
                  <div className="p-4 bg-slate-50 rounded-lg space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-slate-700">معلومات الاختبار:</span>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div className="flex justify-between items-center">
                        ...
                        <Badge variant="secondary" className="bg-blue-100 text-blue-700">
                          {quiz.settings?.use_question_bank
                            ? "يتم تحديدها عشوائياً من بنك الأسئلة"
                            : "عدد الأسئلة غير متاح قبل البدء"
                          }
                        </Badge>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-600">الحد الزمني:</span>
                      ...
                      <div className="flex justify-between items-center">
                        <span className="text-slate-600">إعادة المحاولة:</span>
                        <Badge variant={quiz.settings?.allow_retakes ? "secondary" : "destructive"} className={`${quiz.settings?.allow_retakes ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                          {quiz.settings?.allow_retakes ? "مسموح" : "غير مسموح"}
                        </Badge>
                      </div>
                    </div>
                  </div>
                  ...
                  <Button type="submit" className="w-full" size="lg" disabled={isLoading}>
                      {isLoading ? <Loader2 className="w-4 h-4 animate-spin ml-2" /> : (<><Play className="w-5 h-5 mr-2" />بدء الاختبار</>)}
                  </Button>
                </form>
              </CardContent>
            </Card>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                      ...
                      <div className="flex items-center gap-4 mt-2 md:mt-0">
                        <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">
                          السؤال {currentQuestionIndex + 1} من {assignedQuestions.length}
                        </Badge>
                      </div>
                    </div>
                  </CardHeader>
                    ...
                    {quizSectionsInfo.length > 0 && (
                      <div className="mt-4 pt-4 border-t border-slate-200">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-sm font-medium text-slate-700">توزيع الأسئلة حسب الأقسام:</span>
                        </div>
                        <div className="flex flex-wrap gap-2">
                          {quizSectionsInfo.map((section, index) => (
                            ...
                            >
                              {section.name}: {section.count} أسئلة
                            </Badge>
                          ))}
                        </div>
                      </div>
                      ...
                      <CardHeader className="p-0 pb-4">
                        <div className="flex justify-between items-start mb-4">
                          <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200">
                            {assignedQuestions[currentQuestionIndex]?.points || 1} نقطة
                          </Badge>
                        </div>
                        ...
                        {assignedQuestions[currentQuestionIndex]?.required && getAnswerForQuestion(assignedQuestions[currentQuestionIndex]?.id) === undefined && (
                          <p className="text-red-600 text-sm mt-2 flex items-center gap-1">
                            ...
                            هذا السؤال مطلوب ويجب الإجابة عليه.
                          </p>
                        )}
                      </CardHeader>
                        ...
                        {assignedQuestions[currentQuestionIndex]?.type === "multiple_choice" && (
                          ...
                          >
                            {Array.isArray(assignedQuestions[currentQuestionIndex]?.options) && assignedQuestions[currentQuestionIndex]?.options.map((option, index) => (
                              <div key={index} className="flex items-center space-x-3 space-x-reverse p-3 rounded-lg hover:bg-slate-50 transition-colors">
                                ...
                                <Label
                                  ...
                                  className="flex-1 cursor-pointer text-right text-slate-700 leading-relaxed"
                                >
                                  {option}
                                </Label>
                              </div>
                    ...
                    <div className="flex justify-between mt-8 w-full max-w-3xl mx-auto">
                      ...
                      {currentQuestionIndex < assignedQuestions.length - 1 ? (
                        ...
                        <Button onClick={() => handleSubmit(false)} disabled={quizState === 'submitting'} className="bg-green-600 hover:bg-green-700">
                          {quizState === 'submitting' ? <Loader2 className="w-4 h-4 animate-spin ml-2" /> : <Send className="w-4 h-4 ml-2" />}
                          إرسال
                        </Button>
                      )}
                    </div>
        ...
        {quizState === 'submitting' && (
          ...
          >
            <div className="text-center">
              <Loader2 className="w-12 h-12 animate-spin text-blue-600 mx-auto mb-4" />
              <p className="text-slate-600 text-lg">جاري إرسال إجاباتك...</p>
              <p className="text-slate-500 text-sm mt-2">يرجى الانتظار</p>
            </div>
          </motion.div>
        )}
              ...
              <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 p-4" dir="rtl">
                <Card className="max-w-4xl mx-auto shadow-lg">
                  <CardHeader className="text-center bg-green-50">
                    ...
                    <h1 className="text-3xl font-bold mt-4">نتائج الاختبار</h1>
                    <p className="text-xl text-slate-600">أحسنت! لقد أكملت الاختبار.</p>
                  </CardHeader>
                  <CardContent className="p-8 space-y-6">
                    <div className="text-center text-4xl font-bold">
                      نتيجتك: {finalScore} / {maxScore}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-center">
                      ...
                      <div className="p-4 bg-slate-100 rounded-lg">
                        <p className="text-sm text-slate-500">الوقت المستغرق</p>
                        <p className="text-2xl font-semibold">{formatTime(Math.round((Date.now() - startTime) / 1000))}</p>
                      </div>
                    </div>
                    <div className="space-y-4">
                      <h3 className="text-xl font-semibold">مراجعة الإجابات:</h3>
                      {assignedQuestions.map((q, index) => {
                        ...
                        let userAnswerText = userAnswerObj?.answer || 'لم تجب';
                        ...
                        return (
                          <div key={q.id} className={`p-4 rounded-lg ${isCorrect ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'} border`}>
                            ...
                            <p className="text-sm mt-2">إجابتك: <span className="font-mono">{userAnswerText}</span></p>
                            {!isCorrect && displayCorrectAnswer && (
                              <p className="text-sm text-green-700">الإجابة الصحيحة: <span className="font-mono">{displayCorrectAnswer}</span></p>
                            )}
                            {q.explanation && (
                              <div className="mt-2 pt-2 border-t border-gray-300">
                                <p className="text-sm text-gray-600"><b>توضيح:</b> {q.explanation}</p>
                              </div>
                            )}
                          </div>
                        );
              ...
              <div className="min-h-screen flex items-center justify-center p-4" dir="rtl">
                <Card className="max-w-lg text-center shadow-lg">
                  <CardHeader>
                    ...
                    <CardTitle className="text-3xl mt-4">تم إرسال الاختبار بنجاح!</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-slate-600">شكرًا لك على المشاركة. تم تسجيل إجاباتك.</p>
                  </CardContent>
                </Card>
              </div>
            )}
